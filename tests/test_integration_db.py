...
# --- GARDE-FOU : skip propre si pas de vraie BDD disponible ---

def _real_db_available() -> bool:
    """Évite des timeouts longs en CI si aucune BDD n'est joignable."""
    return bool(os.getenv("RUN_REAL_DB_TESTS")) and DATABASE_URL is not None


requires_real_db = pytest.mark.skipif(
    not _real_db_available(),
    reason="RUN_REAL_DB_TESTS non défini ou DATABASE_URL absente : "
           "tests réels BDD désactivés (voir SKIP_DB_INIT pour les tests unitaires).",
)
...
async def cleanup_database():
    """Nettoie la base de données avant ET après chaque test de ce module."""
...
class TestDatabaseConnection:
    ...
    async def test_postgres_connection(self):
        ...
        try:
            ...
            pytest.fail(f"Connexion à PostgreSQL échouée : {type(e).__name__}: {e}")
...
class TestPredictionLogging:
    ...
    @pytest.mark.real_db
    @requires_real_db
    async def test_predict_success_logging(self, async_client, valid_payload):
        ...
        # Laisse le temps à la background_task d'insérer (fire-and-forget)
        await asyncio.sleep(0.2)

        async with AsyncSessionLocal() as session:
            result = await session.execute(
                select(PredictionLog).order_by(PredictionLog.id.desc()).limit(1)
            )
        ...
        assert log_entry.inputs["EC"] == valid_payload["EC"]

    @pytest.mark.asyncio
    @requires_real_db
    async def test_predict_logging_latency(self, async_client, valid_payload):
        """Vérifie que la réponse API et l'accès en base restent rapides."""
        ...
        await asyncio.sleep(0.2)  # laisse le temps au log en tâche de fond
        ...
        assert db_latency < 1000, f"Lecture en base trop lente : {db_latency:.2f}ms"

    @pytest.mark.asyncio
    @pytest.mark.real_db
    @requires_real_db
    async def test_multiple_predictions_logging(self, async_client, valid_payload):
        ...
        async with AsyncSessionLocal() as session:
            ...
            for log in logs:
                ...
                assert log.inputs["EC"] == valid_payload["EC"]
...
class TestErrorHandling:
    ...
    @pytest.mark.real_db
    @requires_real_db
    async def test_invalid_payload_does_not_log(self, async_client, valid_payload):
        """Vérifie qu'un payload invalide (422) ne tente pas d'insérer de log."""
        invalid_payload = valid_payload.copy()
        del invalid_payload["EC"]
    ...
    @pytest.mark.real_db
    @requires_real_db
    async def test_concurrent_predictions_logging(self, async_client, valid_payload):
...
async def test_db_insertion_performance():
    ...
    assert total_time < 2000, f"Lecture trop lente : {total_time:.2f}ms pour 100 logs"